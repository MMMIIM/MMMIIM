...
<head>
    ...
    <meta name="keywords" content="突破存储瓶颈 新华三X10000 助力河北医科大学第三医院数字化转型" />
    <meta name="description" content="新华三X10000承载PACS业务系统，助力河北医科大学第三医院数字化转型。" />
    ...
    <title>突破存储瓶颈 新华三X10000 助力河北医科大学第三医院数字化转型-新华三集团-H3C</title>
            ...
            <div class="container">
                <div class="row prolist_container">
                    <div class="par_left col-xs-12 col-sm-9 col-md-9">
                        <div class="article">
                            <DIV style="DISPLAY: none" data-longurl="/cn/Products_And_Solution/Storage/Success_Stories/202103/1390806_473262_0.htm"></DIV>
<DIV id=newsContentHead class=clearFix>
<DIV class=subNewsTitle>
<H1>突破存储瓶颈 新华三X10000 助力河北医科大学第三医院数字化转型</H1><!-- content Begin -->
<DIV style="FONT-SIZE: 12px; COLOR: #999999; TEXT-ALIGN: right; LINE-HEIGHT: 24px; MARGIN-RIGHT: 20px">&nbsp;</DIV>
<DIV id=detailBody class=detailBody><DIV><P class=MsoNormal align=left>河北医科大学第三医院是一所知名的省级综合性三级医院，每年收治大量患者。作为一家以骨科见长的医疗机构，院方的日常诊断需要经常使用PACS系统（影像归档和通信系统），然而存储却成为影响PACS系统的一块短板：</P><P class=MsoNormal align=left></P><P class=MsoNormal align=left><B>不够快</B>—院内有众多骨科科室，每天有大量骨科医生调取骨科影像资料进行诊断，当访问高峰时，存储读取性能的不足可能导致系统的卡顿</P><P class=MsoNormal align=left><B>不够大</B>—大型医院每年需要收治大量患者，医学影像信息的增长达到50TB，而现有的系统容量已经捉襟见肘，也无法根据业务进行灵活的扩容</P><P class=MsoNormal align=left></P><P class=MsoNormal align=left>面对未来的业务的需要，院方决定构建全新的双活数据中心，提供高可用的计算资源和存储资源。尤其是针对PACS系统，将传统FC存储切换为分布式存储，充分满足PACS系统对大容量、低并发、高带宽的要求，为以后系统容量扩容提供基础。由紫光股份旗下新华三集团推出的X10000 G3分布式融合存储，同时具备块、文件、对象存储功能，可同时满足PACS系统对文件存储和块存储的要求，实现动态扩容，帮助院方实现了完善的数据全数据生命周期管理。</P><P class=MsoNormal align=center><IMG id="图片 1" src="https://resource.h3c.com/cn/202103/12/20210312_5632361_image001_1390806_473262_0.jpg" width=524 height=931></P><P class=MsoNormal align=left><B>专业服务 实现业务顺利迁移</B></P><P class=MsoNormal></P><P class=MsoNormal>河北医科大学第三医院新建了全新信息机房，并利用原有信息机房做组建双活数据中心，两组数据中心通过裸光纤联通，为医院未来5到10年的发展奠定安全、稳定、可靠的数据IT 基础架构。而在建设过程中，原有的业务系统和数据也需要从传统FC存储迁移到新华三X10000 G3分布式融合存储上。对此，新华三集团出动专业团队，也与院方紧密配合，提供专业的迁移方案，同时发挥X10000 G3分布式融合存储支持iSCSI、以太网、FCOE等多种协议的优势，实现与院方现有的系统进行的完美对接，最终仅用时2天时间就实现了业务的顺利迁移，以最快的速度让新设施数据中心投入运营。</P><P class=MsoNormal></P><P class=MsoNormal align=left><B>强大性能 业务系统零卡顿</B></P><P class=MsoNormal align=left><B></B></P><P class=MsoNormal>作为一家以骨科闻名的专业医院，河北医科大学第三医院日均接诊量超3000人，院内所有科室需要频繁访问PACS业务系统，访问高峰时存储性能不足容易造成系统卡顿。新华三X10000 G3分布式融合存储采用全闪+高容量配置，有效提升存储阵列的 IOPS 和时延表现，而单节点的外部访问也拓展为2Gbps全冗余万兆带宽访问。对比三X10000 G3分布式融合存储落地之前，不同科室对存储的访问速度提升数倍，有效避免业务高发时的访问卡顿，大幅提升医生们在使用PACS系统时的体验。 </P><P class=MsoNormal></P><P class=MsoNormal><B>灵活拓展 全面满足未来需求</B></P><P class=MsoNormal><B></B></P><P class=MsoNormal>随着医院业务系统的不断升级，各个核心业务系统产生了海量的非结构化数据。医院当前PACS系统的数据总量约为 200TB ，根据当前发展趋势预测，年新增数据存储量约为 50 TB。新华三X10000 G3分布式融合存储具有在线扩充节点技术，在院方提出需求后可以实现在当天完成存储资源的快速池化和上线，其业务效率堪比“公有云”。其对非结构化数据的良好支持，在满足PACS系统对文件存储和块存储的要求，更贴合医医院客户的需求。</P><P class=MsoNormal></P><P class=MsoNormal>自从上线至今，新华三X10000 G3分布式融合存储在服务河北医科大学第三医院时实现了数据零丢失，业务零中断。在后续的使用中，新华三集团全生命周期的存储技术服务，大大降低用户的管理与运维成本，受到了院方领带导的赞誉。</P><P class=MsoNormal></P><P class=MsoNormal>新华三集团X10000 G3分布式融合存储在河北医科大学第三医院的首次落地，充分验证了新华三存储产品在医疗行业市场的服务能力。随着业务场景的拓展和技术变革，新华三集团将不断提升X10000 G3分布式融合存储的服务业务表现吗，推动医院信息化建设走向新阶段。</P><P class=MsoNormal></P><P class=MsoNormal></P><P class=MsoNormal><B></B></P><P class=MsoNormal></P><P class=MsoNormal align=left></P></DIV> </DIV><!-- content End -->
