...
import { projectRequirementResponseV2, RESPONSE_ROUTER_V2_VERSION, RESPONSE_ROUTER_V2_IMPLEMENTATION_ID } from '../../backend/src/pipeline/requirement-response-router-v2.js';
...
const ROUTER_FILE = path.join(ROOT, 'backend/src/pipeline/requirement-response-router-v2.js');
...
function buildRows(reference, v2, sealedKey) {
  ...
  return reference.requirements.map(requirement => {
    const router = projectRequirementResponseV2(requirement, { projection_version: RESPONSE_ROUTER_V2_VERSION });
...
function main() {
  ...
  const runId = `V43-RR-V2-V3-REPLAY-${sha256(`${referenceSha}:${sha256(fs.readFileSync(ROUTER_FILE))}:${RESPONSE_ROUTER_V2_VERSION}`).slice(0, 16)}`;
  ...
  const full = {
    artifact_type: 'V43_RESPONSE_ROUTER_V2_VS_REFERENCE_V3_FULL_CORPUS_EVAL', artifact_version: 'v3-replay', run_id: runId, generated_at: generatedAt,
    ...
    router_identity: { path: path.relative(ROOT, ROUTER_FILE), sha256: routerSha, projection_version: RESPONSE_ROUTER_V2_VERSION, implementation_id: RESPONSE_ROUTER_V2_IMPLEMENTATION_ID },
  ...
  const confusion = { artifact_type: 'V43_RESPONSE_ROUTER_V2_VS_REFERENCE_V3_CONFUSION_MATRIX', artifact_version: 'v3-replay', run_id: runId, reference_v3_sha256: referenceSha, router_sha256: routerSha, case_count: rows.length, primary_response_mode: full.overall_primary_mode, risk_tier: { labels: RISKS, matrix_router_rows_reference_columns: confusionMatrix(rows, row => row.router.risk_tier, row => row.v3.risk_tier, RISKS), agreement: fieldAgreements.risk_tier }, cross_cutting: fieldAgreements };
  const failureArtifact = { artifact_type: 'V43_RESPONSE_ROUTER_V2_VS_REFERENCE_V3_FAILURE_FAMILIES', artifact_version: 'v3-replay', run_id: runId, reference_v3_sha256: referenceSha, router_sha256: routerSha, case_count: rows.length, families: failureFamilies, disagreement_count: modeDisagreements.length, disagreement_case_ids: modeDisagreements.map(row => row.case_id) };
  const v2Drift = drift(v2, rows); v2Drift.artifact_type = 'V43_RESPONSE_REFERENCE_V2_TO_V3_DRIFT'; v2Drift.artifact_version = 'v2-to-v3'; v2Drift.run_id = runId; v2Drift.reference_v2_sha256 = v2Sha; v2Drift.reference_v3_sha256 = referenceSha;
  const checkpoint = { checkpoint: 'V43_RESPONSE_ROUTER_V2_VS_REFERENCE_V3_CHECKPOINT', run_id: runId, generated_at: generatedAt, reference_v3_identity: { locator: reference.locator, sha256: referenceSha, rows: requirements.length, unique_case_ids: new Set(requirements.map(row => row.case_id)).size, cohort_counts: EXPECTED_COHORTS }, router_v2_identity: { path: path.relative(ROOT, ROUTER_FILE), sha256: routerSha, sealed_projection_match_count: sealedMatchCount, sealed_projection_match_rate: pct(sealedMatchCount, rows.length) }, metrics: { overall_primary_mode_agreement: full.overall_primary_mode, cross_cutting: fieldAgreements, abstention: full.abstention, disagreement_count: modeDisagreements.length, failure_families: failureFamilies.map(({ id, count, rate }) => ({ id, count, rate })), v2_to_v3_primary_mode_transitions: v2Drift.primary_mode_transitions }, side_effects: { PROVIDER_CALLS: 0, LLM_CALLS: 0, PRODUCTION_DB_WRITES: 0, REFERENCE_V2_MUTATIONS: 0, REFERENCE_V3_MUTATIONS: 0, ROUTER_V1_MUTATIONS: 0, ROUTER_V2_MUTATIONS: 0, REQUIREMENT_MUTATIONS: 0, FACT_MUTATIONS: 0, MAPPING: 0, CLAIM: 0, WRITER: 0, COMMIT: 0, PUSH: 0, MERGE: 0, DEPLOY: 0 }, status: 'READY_FOR_GPT_RESPONSE_ROUTER_V2_VS_REFERENCE_V3_ADJUDICATION' };
  fs.writeFileSync(path.join(DOCS, 'V43_RESPONSE_ROUTER_V2_VS_REFERENCE_V3_FULL_CORPUS_EVAL.json'), `${JSON.stringify({ ...full, rows }, null, 2)}\n`);
  fs.writeFileSync(path.join(DOCS, 'V43_RESPONSE_ROUTER_V2_VS_REFERENCE_V3_CONFUSION_MATRIX.json'), `${JSON.stringify(confusion, null, 2)}\n`);
  fs.writeFileSync(path.join(DOCS, 'V43_RESPONSE_ROUTER_V2_VS_REFERENCE_V3_FAILURE_FAMILIES.json'), `${JSON.stringify(failureArtifact, null, 2)}\n`);
  fs.writeFileSync(path.join(DOCS, 'V43_RESPONSE_REFERENCE_V2_TO_V3_DRIFT.json'), `${JSON.stringify(v2Drift, null, 2)}\n`);
  const md = [
    '# V43 Response Router V2 vs Frozen Reference V3', '', `- run_id: ${runId}`, `- reference_v3: ${reference.locator}`, `- reference_v3_sha256: ${referenceSha}`, `- cases: ${rows.length}`, `- Router V2: ${path.relative(ROOT, ROUTER_FILE)}`, `- router_sha256: ${routerSha}`, `- primary agreement: ${full.overall_primary_mode.agreement_count}/${rows.length} (${full.overall_primary_mode.agreement_rate})`, `- disagreements: ${modeDisagreements.length}`, `- genuine ambiguous reference cases: ${v3Ambiguous.length}`, `- Router NEED_REVIEW: ${routerNeedReview.length}`, '', '## Per-mode metrics', '', ...Object.entries(full.overall_primary_mode.per_mode_precision_recall).map(([mode, m]) => `- ${mode}: precision=${m.precision}, recall=${m.recall}, TP=${m.true_positive}, FP=${m.false_positive}, FN=${m.false_negative}`), '', '## Safety boundaries', '', `- P0 compliance escape: ${p0Compliance.length}`, `- high-risk evidence false negative: ${highRiskEvidence.length}`, `- future commitment → evidence: ${full.safety_boundaries.future_commitment_as_existing_evidence.length}`, `- project solution → evidence: ${full.safety_boundaries.project_solution_as_enterprise_evidence.length}`, `- evidence → solution: ${full.safety_boundaries.evidence_as_solution.length}`, `- commitment → compliance: ${full.safety_boundaries.commitment_as_compliance.length}`, `- compliance → commitment: ${full.safety_boundaries.compliance_as_commitment.length}`, '', '## Side effects', '', '- Provider calls: 0', '- LLM calls: 0', '- Production DB writes: 0', '- Reference V2/V3 mutations: 0', '- Router V1/V2 mutations: 0', '- Commit/push/merge/deploy: 0', '', 'Failure families are mechanically assigned from frozen V3 labels and Router output; no semantic truth was inferred by Codex.'
  ].join('\n');
  fs.writeFileSync(path.join(DOCS, 'V43_RESPONSE_ROUTER_V2_VS_REFERENCE_V3_CHECKPOINT.md'), `${md}\n`);
