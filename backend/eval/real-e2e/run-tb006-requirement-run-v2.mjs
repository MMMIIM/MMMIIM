...
const runId = 'REAL_E2E_CASE_01_REQUIREMENT_RUN_V2';
const storageRoot = join(root, 'data', 'eval', 'real-e2e-case-01-v2-storage');
const reportPath = join(root, 'docs', 'handoff', 'V43_HANDOFF_REAL_E2E_CASE_01_REQUIREMENT_HUMAN_GATE', '03_TB006_REQUIREMENT_RUN_V2.json');
...
try {
  ...
  await mkdir(join(root, 'docs', 'handoff', 'V43_HANDOFF_REAL_E2E_CASE_01_REQUIREMENT_HUMAN_GATE'), { recursive: true });
  ...
  const project = await repository.createProjectWithOwner({
    name: `V43 Real E2E Case 01 Requirement Run V2 ${runId}`,
    deadline: null,
    owner: actor
  });
