---
doc_id: "CCV2-U06-02"
title: "数据迁移与交换-MIGRATION_VERIFICATION_RECORD"
...
requirement_themes: ["U06"]
matrix_theme: "U06"
evidence_slot: "MIGRATION_VERIFICATION_RECORD"
...
# 数据迁移与交换-MIGRATION_VERIFICATION_RECORD

## 记录对象

本记录属于澄川数智科技有限公司（Synthetic Demo Company）的受控开发企业材料，面向数据迁移与交换主题。

## 可核验记录

1. 2026-04迁移验证记录包含860万条结构化记录，分4批完成。
2. 验证阶段抽样比对源端和目标端记录数、关键字段值和唯一键。
3. 该次验证发现127条源数据日期格式异常，修正后重新导入成功。
4. 验证记录不包含非结构化影像迁移，不能外推至影像文件迁移能力。
