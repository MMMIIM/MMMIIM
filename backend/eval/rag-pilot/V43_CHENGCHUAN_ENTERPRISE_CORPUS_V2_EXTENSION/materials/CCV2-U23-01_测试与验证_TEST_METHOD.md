...
1. 测试方法覆盖单元、接口、功能、性能、安全、兼容和用户验收支持。
2. 缺陷按阻塞、严重、一般、建议四级管理。
3. 阻塞和严重缺陷未关闭时发布审批默认不通过。
4. 测试证据包含用例、执行结果、缺陷、修复版本和复测结果。
