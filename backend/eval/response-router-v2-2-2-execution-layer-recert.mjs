...
const OUTPUT = path.join(DOCS, 'V43_RESPONSE_ROUTER_V2_2_2_EXECUTION_LAYER_RECERT.json');
...
function main() {
  ...
  const unknownResponseIds = decisions.filter(row => !ids.includes(row.requirement_id)).map(row => row.requirement_id);
  ...
  const recert = {
    checkpoint: 'V43_RESPONSE_ROUTER_V2_2_2_EXECUTION_LAYER_RECERT',
