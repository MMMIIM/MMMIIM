...
import { RESPONSE_ROUTER_V223_VERSION, RESPONSE_ROUTER_V223_IMPLEMENTATION_ID } from '../src/pipeline/requirement-response-router-v2-2-3.js';

const ROOT = path.resolve(process.cwd());
const DOCS = path.join(ROOT, 'docs');
const INPUT = path.join(DOCS, 'V43_REQUIREMENT_GOLD_V2_SIX_TENDER_CANONICAL_INPUT.json');
const OUTPUT = path.join(DOCS, 'V43_RESPONSE_ROUTER_V2_2_3_EXECUTION_LAYER_RECERT.json');
...
const main = () => {
  ...
  if (sha(raw) !== EXPECTED_SHA) throw new Error('CANONICAL_INPUT_SHA_MISMATCH');
  const source = JSON.parse(raw.toString('utf8'));
  const requirements = list(source.requirements);
  const ids = requirements.map(row => row.canonical_requirement_id);
  if (requirements.length !== 1009 || new Set(ids).size !== 1009) throw new Error('CANONICAL_INPUT_IDENTITY_MISMATCH');
  ...
  const units = buildResponseUnitsV1(matrix.map(row => ({ ...row.response_decision, readiness_status: row.readiness_status, section_key: row.requirement_id })));
  const packetSamples = [];
  ...
  const recert = {
    checkpoint: 'V43_RESPONSE_ROUTER_V2_2_3_EXECUTION_LAYER_RECERT',
    generated_at: new Date().toISOString(),
    canonical_input: { path: path.relative(ROOT, INPUT), sha256: sha(raw), count: requirements.length, tender_counts: Object.fromEntries([...new Set(requirements.map(row => row.tender_id))].map(tender => [tender, requirements.filter(row => row.tender_id === tender).length])) },
    router_identity: { projection_version: decisions[0]?.projection_version || null, implementation_id: decisions[0]?.implementation_id || null, expected_version: RESPONSE_ROUTER_V223_VERSION, expected_implementation_id: RESPONSE_ROUTER_V223_IMPLEMENTATION_ID },
    execution: {
      ...
      response_unit_ids_unknown: [...matrixIds].filter(id => !responseIds.has(id)).length,
    ...
    side_effects: { provider_calls: 0, llm_calls: 0, production_db_writes: 0, gold_mutations: 0, requirement_mutations: 0, production_semantic_changes: 0 },
    status: matrix.length === 1009 && requiredIds.size === [...requiredIds].filter(id => matrixIds.has(id)).length && writerDirectCandidates.length === 0 && enterpriseAuthorityCandidates.length === 0 && decisions.every(row => row.requirement_id && row.projection_version === RESPONSE_ROUTER_V223_VERSION) ? 'PASS' : 'FAIL'
  };
  fs.writeFileSync(OUTPUT, `${JSON.stringify(recert, null, 2)}\n`);
  console.log(JSON.stringify({ status: recert.status, matrix_rows: matrix.length, response_required: requiredIds.size, units: units.length, router: recert.router_identity, provider_calls: 0, db_writes: 0 }, null, 2));
};
